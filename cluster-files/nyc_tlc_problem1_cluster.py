...
This script reads 12 months of NYC TLC data (January-December 2021)
directly from S3, combines them, and calculates daily summaries.
"""
...
def create_spark_session(master_url):
    ...
    spark = (
        ...
        .config("spark.driver.port", "37979")
        ...
        .config("spark.cores.max", "4")  # Use all available cores across cluster
...
def solve_problem1(spark, data_files):
    ...
    if pandas_df.empty:
        raise ValueError("No daily summaries were produced; cannot write a meaningful result.")
    ...
    first_date = pandas_df.iloc[-1]
    last_date = pandas_df.iloc[0]
    print(f"Date range: {int(first_date['dt_month']):02d}/{int(first_date['dt_day']):02d}/{int(first_date['dt_year'])} to "
          f"{int(last_date['dt_month']):02d}/{int(last_date['dt_day']):02d}/{int(last_date['dt_year'])}")
...
def main():
    ...
    print("NYC Taxi Trip Data Analysis (January-December 2021)")
    print("=" * 70)
    ...
    # Change this list only if your assignment requires a different period.
    ...
    # Solve Problem 1
    try:
        logger.info("Starting Problem 1 analysis with S3 data files")
        solve_problem1(spark, data_files)
        success = True
